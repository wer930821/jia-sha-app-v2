...
const state={category:'全部',budget:null,maxDistanceKm:2,transport:'walk',excluded:[],openOnly:true,selectedId:null,restaurants:[...demoRestaurants],position:null,loading:false,isLive:false,lastFetchedRadiusKm:0};
const ids=['categoryChips','transportChips','exclusionChips','openOnly','chooseButton','resultCount','notice','resultCard','nearbyCount','restaurantList','locationButton','locationTitle','locationText','budgetInput','unlimitedBudget','distanceInput'];
...
function inferInfo(place){
  ...
  if(breakfastWords.test(text)||breakfastCuisine.test(cuisine)||type==='breakfast_restaurant'||type==='brunch_restaurant') return {type:'早餐',group:'早餐'};
...
async function fetchOsmRestaurants(lat,lon,radiusKm){
  ...
  const response=await fetch(`/api/nearby?lat=${encodeURIComponent(lat)}&lng=${encodeURIComponent(lon)}&radius=${radius}`,{headers:{Accept:'application/json'}});
...
function renderResult(){
  ...
  els.resultCard.classList.remove('hidden');document.getElementById('changeButton').onclick=chooseRestaurant;document.getElementById('mapButton').onclick=()=>window.open(mapUrl(x),'_blank','noopener,noreferrer');
}
function renderList(){
...
function render(){renderControls();renderList();if(state.selectedId&&!getFiltered().some(x=>x.id===state.selectedId))state.selectedId=null;renderResult();}
...
els.openOnly.onchange=e=>{state.openOnly=e.target.checked;render();};els.chooseButton.onclick=chooseRestaurant;els.locationButton.onclick=()=>state.position?searchAtPosition():useLocation();render();
