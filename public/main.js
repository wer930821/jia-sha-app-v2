...
const state={category:'全部',budget:null,people:1,maxDistanceKm:2,transport:'walk',excluded:[],openOnly:false,selectedId:null,restaurants:[...demoRestaurants],position:null,loading:false,isLive:false,lastFetchedRadiusKm:0,map:null,mapLoaded:false,mapMarkers:[],userMarker:null,searchStats:{rawCount:0,returnedCount:0,normalizedCount:0},menuSearches:new Map()};
const ids=['map','mapStatus','fitMapButton','categoryChips','transportChips','exclusionChips','openOnly','chooseButton','resultCount','notice','resultCard','nearbyCount','restaurantList','locationButton','locationTitle','locationText','budgetInput','unlimitedBudget','distanceInput','peopleInput','budgetSummary','budgetSuggestion'];
...
function perPersonBudget(){return state.budget===null?null:Math.floor(state.budget/Math.max(1,state.people));}
function budgetText(){return state.budget===null?'不限預算':`總預算 $${state.budget}（每人約 $${perPersonBudget()}）`;}
...
function budgetMatches(x){
  if(state.budget===null||!x.menu?.length)return true;
  const minPrice=Math.min(...x.menu.map(item=>Number(item.price)||Infinity));
  return Number.isFinite(minPrice)&&minPrice*Math.max(1,state.people)<=state.budget;
}
const exclusionPatterns={
  飯:/(飯|便當|餐盒|丼|燴飯|自助餐)/i,
  麵:/(麵|米粉|冬粉|板條|河粉|拉麵|烏龍)/i,
  水餃:/(水餃|鍋貼|煎餃|蒸餃|餃子)/i,
  早餐:breakfastWords,
  小吃:/(小吃|肉圓|臭豆腐|滷味|關東煮|蚵仔煎|甜不辣)/i,
  炸物:friedWords,
  火鍋:/(火鍋|鍋物|涮涮鍋|麻辣鍋)/i,
  燒肉:/(燒肉|烤肉|炭火)/i,
  牛排:/(牛排|steak)/i,
  ...
  飲料:drinkWords,
  甜點:dessertWords,
  咖啡:/(咖啡|coffee|cafe|café)/i
};
function exclusionMatches(x){
  const text=`${x.name||''} ${x.type||''} ${x.group||''} ${(x.tags||[]).join(' ')}`;
  return state.excluded.some(label=>x.type===label||x.group===label||exclusionPatterns[label]?.test(text));
}
function getFiltered(){return state.restaurants.filter(x=>!closedReports.has(x.id)&&(state.category==='全部'||x.group===state.category)&&budgetMatches(x)&&x.distanceKm<=state.maxDistanceKm&&!exclusionMatches(x)&&(!mealPeriods[state.category]||x.mealOpen!==false)&&(!state.openOnly||x.open!==false));}
...
function renderControls(){
  ...
  els.openOnly.checked=state.openOnly; els.unlimitedBudget.checked=state.budget===null; els.budgetInput.disabled=state.budget===null; els.budgetInput.value=state.budget??''; els.distanceInput.value=state.maxDistanceKm; els.peopleInput.value=state.people;
  const pp=perPersonBudget();
  els.budgetSummary.textContent=state.budget===null?`${state.people} 人 · 不限預算`:`${state.people} 人 · 總預算 $${state.budget} · 每人約 $${pp}`;
  els.budgetSuggestion.textContent=state.budget===null?'設定總預算後，會換算每人可用金額。':budgetSuggestionText(pp);
}

function budgetSuggestionText(pp){
  if(pp<60)return '預算參考：可優先找早餐、飲料或單點小吃；實際價格仍以真實菜單為準。';
  if(pp<120)return '預算參考：可優先找早餐、小吃、麵飯或便當；實際價格仍以真實菜單為準。';
  if(pp<250)return '預算參考：多數一般午餐、晚餐或簡餐都有機會；實際價格仍以真實菜單為準。';
  return '預算參考：可選範圍較廣；有真實菜單的店會直接幫你檢查品項。';
}
...
function renderResult(){
  ...
  const pp=perPersonBudget(); const within=x.menu?.length?(pp===null?x.menu.length:x.menu.filter(i=>i.price<=pp).length):0;
  els.resultCard.innerHTML=`<div class="result-eyebrow">今天就吃這間</div><div class="result-heading"><div><h2>${escapeHtml(x.name)}</h2><p>${escapeHtml(x.type)} · ${x.distanceKm} km</p></div><div class="rating">附近</div></div><div class="price-line">${x.menu?.length?(state.budget===null?'已有真實菜單資料':`${state.people} 人總預算 $${state.budget} · 每人約 $${pp} · ${within} 項真實餐點在每人預算內`):`${state.people} 人${state.budget===null?' · 不限預算':` · 總預算 $${state.budget} · 每人約 $${pp}`} · 真實價格尚未收錄`}</div><div class="meta-grid"><div>🧭 ${transportText()}約 ${travelMinutes(x.distanceKm)} 分鐘</div><div>🕒 ${x.open===true?'目前營業':x.open===false?'目前休息':'目前營業狀態未確認'}</div><div>📅 ${x.openingHours?escapeHtml(x.openingHours):'營業時間未收錄'}</div></div><div class="address">${escapeHtml(x.address)}</div><div class="tag-row">${x.tags.map(t=>`<span>${escapeHtml(t)}</span>`).join('')}</div>${menuHtml(x)}<div class="action-row"><button class="secondary-button" data-action="change" type="button">🔄 換一家</button><button class="map-button" data-action="map" type="button">🧭 確認店家定位</button></div>`;
...
els.peopleInput.onchange=e=>{state.people=Math.min(20,Math.max(1,Math.round(Number(e.target.value)||1)));render();};
